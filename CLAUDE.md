...
- **Dark mode**: automático via `var(--ds-*)` — zero lógica de tema manual; brand colors (`#0C66E4`/`#5E4DB2`) ficam como hex (funcionam em ambos os temas); só backgrounds e foregrounds de texto precisam de token. Tokens semânticos em `tokens.js`: `bgDanger/bgSuccess/bgNeutral/bgBrand/textOnBold/textDanger/textSuccess/textWarning/textBrand`
...
- **i18n qualidade**: após gerar traduções com agente, sempre rodar revisão semântica separada por idioma — erros comuns: verbos onde deveria haver substantivos (hu), terminologia PM incorreta (ko: "critical path" ≠ jargão de engenharia civil), anglicismos desnecessários (cs/tr/sk)
- **i18n paridade**: validar com `node -e 'const fs=require("fs"),p="src/i18n/locales/",keys=(o,r="")=>Object.entries(o).flatMap(([k,v])=>typeof v==="object"?keys(v,r+k+"."):r+k),en=new Set(keys(JSON.parse(fs.readFileSync(p+"en.json"))));let bad=0;for(const f of fs.readdirSync(p).filter(f=>f.endsWith(".json"))){const k=new Set(keys(JSON.parse(fs.readFileSync(p+f))));const m=[...en].filter(x=>!k.has(x));if(m.length){console.log("⚠",f,m);bad++;}}console.log(bad===0?"✅ OK":"❌ "+bad);'` dentro de `static/gantt-app/`
- **i18n**: **24 JSONs** em `src/i18n/locales/` (conjunto Atlassian Cloud: en, en-GB, pt-BR, es, fr, de, it, nl, pl, ja, ko, zh, zh-TW, cs, da, fi, hu, nb, ru, sv, tr, et, is, sk) — **fonte = en.json (137 chaves)**; registrados em `src/i18n/index.js` (`resources` + array `SUPPORTED` + `applyLocale` que mapeia locale Atlassian → idioma, ex. `zh_Hant`→`zh-TW`). Ao adicionar chave: inserir nos 24 e validar paridade com o script de chaves (compara cada locale contra en.json, não só `JSON.parse`). Âncora p/ inserir: linha `  "jira": {`. Sufixo de dia "d" e a palavra "Burndown" ficam literais (não traduzidos)
...
- **Git**: `.gitignore` na raiz exclui `node_modules/` e `static/gantt-app/node_modules/` — se ausente, rodar `git rm -r --cached node_modules/` antes do primeiro commit; `dist/` é rastreado intencionalmente (builds deployados ficam no histórico)
